...
uploaded_md_file = st.file_uploader(
    "Upload your metadata json file", type="json")
...
if uploaded_file is not None:
    st.text("File name: {}\nType: {}\nSize: {}".format(uploaded_file.name, uploaded_file.type, uploaded_file.size))
if st.button("Load Data"):
    m = MultipartEncoder(
        fields={"file": (uploaded_file.name, uploaded_file.read(), uploaded_file.type)}
    )
    headers = {
        'Authorization': 'Bearer ' + st.session_state["token"],
    ...
    proj_data_response = requests.post(
        urls.post_metadata+'?project_id='+st.session_state['proj_id']+'&file_name='+uploaded_file.name, 
        headers=headers, 
        data=m
        )
